...
bool bmpImage::fetch() {
    ...
    colormap = (PalColor*) new U32[256];
    set_colors((Unsigned32*) colormap,256);
    ...
    pixel_type(depth>3 ? ARGB32 : RGB);
    uchar *array =alloc_pixels(w(), h(), pixel_type());
    //alloc_array = 1;
	...
	case 4 : // 16-color
		...
		if (bit == 0xf0) {
		    ...
		    *ptr++ = colormap[(temp >> 4) & 15].c[2];
		    *ptr++ = colormap[(temp >> 4) & 15].c[1];
		    *ptr++ = colormap[(temp >> 4) & 15].c[0];
		    ...
		    // Copy the color value...
		    *ptr++ = colormap[temp & 15].c[2];
		    *ptr++ = colormap[temp & 15].c[1];
		    *ptr++ = colormap[temp & 15].c[0];
		}
	...
	case 8 : // 256-color
		...
		// Copy the color value...
		ptr[0] = colormap[temp].c[2];
		ptr[1] = colormap[temp].c[1];
		ptr[2] = colormap[temp].c[0];
	    }
  ...
  if (!pixels()) {fclose(bmpFile); return false;}
  return true;
}
  ...
  void bmpImage::read() {
      ...
      {GSave gsave;
      make_current();
      drawimage(pixels(), pixel_type(), Rectangle(w(), h()), ld());}
