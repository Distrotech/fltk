...
int Fl_Roller::handle(int event) {
  static int ipos;
  int newpos = horizontal() ? fltk::event_x() : fltk::event_y();
  switch (event) {
  case FL_PUSH:
...
void Fl_Roller::draw() {
  ...
  int W = w()-fltk::box_dw(box())-1;
  int H = h()-fltk::box_dh(box())-1;
  if (W<=0 || H <=0) return;
  ...
  if (fltk::focus() == this) draw_focus(FL_THIN_UP_FRAME, x(), y(), w(), h());
}
