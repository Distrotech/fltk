...
void Fl_Scrollbar::timeout_cb(void* v) {
  ...
  s->increment_cb();
  fltk::add_timeout(REPEAT, timeout_cb, s);
}
...
int Fl_Scrollbar::handle(int event) {
  ...
  if (relx < 0) area = 1;
    ...
    if (fltk::event_button() == FL_MIDDLE_MOUSE) area = 8;
  ...
  case FL_RELEASE:
    ...
    if (pushed_) {
      fltk::remove_timeout(timeout_cb, this);
      pushed_ = 0;
    }
    handle_release();
    return 1;
  case FL_PUSH:
    ...
    if (pushed_) {
      handle_push();
      fltk::add_timeout(INITIALREPEAT, timeout_cb, this);
      increment_cb();
      damage(FL_DAMAGE_ALL);
      return 1;
    }
  ...
  case FL_MOUSEWHEEL :
    if (horizontal()) {
      if (fltk::e_dx==0) return 0;
      handle_drag(clamp(value() + linesize_ * fltk::e_dx));
      return 1;
    } else {
      if (fltk::e_dy==0) return 0;
      handle_drag(clamp(value() + linesize_ * fltk::e_dy));
      return 1;
    }
  case FL_SHORTCUT:
  case FL_KEYBOARD: {
...
void Fl_Scrollbar::draw() {
  ...
  int X = x()+fltk::box_dx(box());
  int Y = y()+fltk::box_dy(box());
  int W = w()-fltk::box_dw(box());
  int H = h()-fltk::box_dh(box());
  if (horizontal()) {
    ...
    if (damage()&FL_DAMAGE_ALL) {
      ...
      if (fltk::scheme_ && !strcmp(fltk::scheme_, "gtk+")) {
