...
//i.bitmap_pad = 8;
  ...
  if (pfv->scanline_pad & 7 || (n&(n-1)))
    fltk::fatal("Can't do scanline_pad of %d",pfv->scanline_pad);
...
#  if USE_COLORMAP
  ...
  if (!fl_visual->red_mask)
    fltk::fatal("Can't do %d bits_per_pixel colormap",xi.bits_per_pixel);
#  endif
...
#  endif
  ...
  case 3:
    ...
    if (rs == 0 && gs == 8 && bs == 16) {
      ...
      fltk::fatal("Can't do arbitrary 24bit color");
    }
    break;
  ...
  default:
      fltk::fatal("Can't do %d bits_per_pixel",xi.bits_per_pixel);
  }
