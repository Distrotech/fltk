...
Fl_GIF_Image::Fl_GIF_Image(const char *infname) : Fl_Pixmap((char *const*)0) {
  ...
  if ((GifFile = fl_fopen(infname, "rb")) == NULL) {
    fltk::error("Fl_GIF_Image: Unable to open %s!", infname);
    return;
  }
  ...
  if (b[0]!='G' || b[1]!='I' || b[2] != 'F') {
    fclose(GifFile);
    fltk::error("Fl_GIF_Image: %s is not a GIF file.\n", infname);
    return;
  }
  if (b[3]!='8' || b[4]>'9' || b[5]!= 'a')
    fltk::warning("%s is version %c%c%c.",infname,b[3],b[4],b[5]);
  }
  ...
  if (HasColormap) {
    ...
    fltk::warning("%s does not have a colormap.", infname);
    for (int i = 0; i < ColorMapSize; i++)
  ...
  for (;;) {
    ...
    if (i<0) {
      fclose(GifFile);
      fltk::error("Fl_GIF_Image: %s - unexpected EOF",infname); 
      return;
    }
    int blocklen;
	...
	;
        ...
        fltk::warning("%s: unknown gif extension 0x%02x.", infname, ch);
      }
	...
	for (i=0; i < n; i++) {	
      ...
      fltk::warning("%s: unknown gif code 0x%02x", infname, i);
      blocklen = 0;
    }
 ...
 * that must be correctly skipped over. */
    ...
    else {fltk::error("Fl_GIF_Image: %s - LZW Barf!", infname); break;}
