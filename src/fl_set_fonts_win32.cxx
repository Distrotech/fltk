...
//n[fl_unicode2utf((xchar*)lpelf->lfFaceName, l, n)] = 0;
  ...
  for (int i=0; i<FL_FREE_FONT; i++) // skip if one of our built-in fonts
	  if (!strcmp(fltk::get_font_name((Fl_Font)i),n)) {free(n);return 1;}
  char buffer[LF_FACESIZE + 1];
  strcpy(buffer+1, n);
  buffer[0] = ' '; fltk::set_font((Fl_Font)(fl_free_font++), strdup(buffer));
  if (lpelf->lfWeight <= 400)
    buffer[0] = 'B', fltk::set_font((Fl_Font)(fl_free_font++), strdup(buffer));
  buffer[0] = 'I'; fltk::set_font((Fl_Font)(fl_free_font++), strdup(buffer));
  if (lpelf->lfWeight <= 400)
    buffer[0] = 'P', fltk::set_font((Fl_Font)(fl_free_font++), strdup(buffer));
  free(n);
  return 1;
} /* enumcbw */
...
int
fltk::get_font_sizes(Fl_Font fnum, int*& sizep) {
  nbSize = 0;
