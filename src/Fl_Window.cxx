...
void fltk::Window::_Fl_Window() {
  ...
  callback((fltk::Callback*)default_callback);
}
...
fltk::Window *fltk::Widget::window() const {
  for (fltk::Widget *o = parent(); o; o = o->parent())
    if (o->type() >= FL_WINDOW) return (fltk::Window*)o;
  return 0;
}
...
void fltk::Window::draw() {
  ...
  if (!parent() && resizable() && (!size_range_set || minh!=maxh || minw!=maxw)) {
    int dx = fltk::box_dw(box())-fltk::box_dx(box());
    int dy = fltk::box_dh(box())-fltk::box_dy(box());
    if (dx<=0) dx = 1;
    if (dy<=0) dy = 1;
...
# if defined(USE_CAIRO)
  fltk::cairo_make_current(this); // checkout if an update is necessary
# endif
}
...
// the fltk::atclose pointer is provided for back compatibility.  You
...
void fltk::default_atclose(fltk::Window* window, void* v) {
  window->hide();
  fltk::Widget::default_callback(window, v); // put on fltk::read_queue()
}
/** Back compatibility: default window callback handler \see fltk::set_atclose() */
void (*fltk::atclose)(fltk::Window*, void*) = default_atclose;
...
/**  Returns the last window that was made current. \see fltk::Window::make_current() */
fltk::Window *fltk::Window::current() {
  return current_;
}
