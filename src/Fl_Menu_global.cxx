...
static int handler(int e) {
  if (e != FL_SHORTCUT || fltk::modal()) return 0;
  fltk::first_window(the_widget->window());
...
/**
  ...
  fltk::add_handler().  This Fl_Menu_ widget does not
...
void Fl_Menu_::global() {
  if (!the_widget) fltk::add_handler(handler);
  the_widget = this;
}
