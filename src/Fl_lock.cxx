...
/*
   ...
   fltk::lock() - recursive lock.  You must call this before the
   first call to fltk::wait()/run() to initialize the thread
   system. The lock is locked all the time except when
   fltk::wait() is waiting for events.

   fltk::unlock() - release the recursive lock.

   fltk::awake(void*) - Causes fltk::wait() to return (with the lock
   locked) even if there are no events ready.

   fltk::awake(void (*cb)(void *), void*) - Call a function
   in the main thread from within another thread of execution.

   fltk::thread_message() - returns an argument sent to an
   fltk::awake() call, or returns NULL if none.  WARNING: the
   current implementation only has a one-entry queue and only
   returns the most recent value!
*/

#ifndef FL_DOXYGEN
Fl_Awake_Handler *fltk::awake_ring_;
void **fltk::awake_data_;
int fltk::awake_ring_size_;
int fltk::awake_ring_head_;
int fltk::awake_ring_tail_;
#endif
...
int fltk::add_awake_handler_(Fl_Awake_Handler func, void *data)
{
  int ret = 0;
  lock_ring();
  if (!awake_ring_) {
...
int fltk::get_awake_handler_(Fl_Awake_Handler &func, void *&data)
{
  int ret = 0;
  lock_ring();
...
*/
int fltk::awake(Fl_Awake_Handler func, void *data) {
...
/** \fn void fltk::awake(void* msg)
    ...
    In the context of a threaded application, a call to fltk::awake() with no
    argument will trigger event loop handling in the main thread. Since
    it is not possible to call fltk::flush() from a subsidiary thread,
    fltk::awake() is the best (and only, really) substitute.
...
// Pipe for thread messaging via fltk::awake()...
static int thread_filedes[2];

// Mutex and state information for fltk::lock() and fltk::unlock()...
...
void fltk::lock() {
  if (!thread_filedes[1]) {
    ...
    // fltk::awake() from a thread will "wake up" the main thread in
    // fltk::wait().
    fltk::add_fd(thread_filedes[0], FL_READ, thread_awake_cb);
