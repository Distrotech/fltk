...
int fl_scandir(const char *dirname, struct dirent ***namelist,
  ...
  { /* Create a block to limit the scope while we find the initial "wide" filename */
	...
	unsigned wlen = fltk3::utf8toUtf16(findIn, (unsigned)strlen(findIn), NULL, 0); /* Pass NULL to query length */
	wlen++; /* add a little extra for termination etc. */
	wbuf = (unsigned short*)malloc(sizeof(unsigned short)*wlen);
	wlen = fltk3::utf8toUtf16(findIn, (unsigned)strlen(findIn), wbuf, wlen); /* actually convert the filename */
