...
fltk::Widget* Fl_Tooltip::widget_ = 0;
...
void Fl_TooltipBox::layout() {
  ...
  int ox = fltk::event_x_root();
  int oy = Y + H+2;
  for (fltk::Widget* p = Fl_Tooltip::current(); p; p = p->window()) {
    oy += p->y();
  }
...
*/
void Fl_Tooltip::enter_(fltk::Widget* w) {
#ifdef DEBUG
...
*/
void Fl_Tooltip::current(fltk::Widget* w) {
#ifdef DEBUG
...
void Fl_Tooltip::exit_(fltk::Widget *w) {
#ifdef DEBUG
  ...
  if (recent_tooltip) {
    if (fltk::event_state() & FL_BUTTONS) recent_tooltip = 0;
    else fltk::add_timeout(Fl_Tooltip::hoverdelay(), recent_timeout);
  }
}
...
/**
  ...
  of your widget. Call this after setting fltk::belowmouse() to
...
*/
void Fl_Tooltip::enter_area(fltk::Widget* wid, int x,int y,int w,int h, const char* t)
{
  (void)x;
  (void)w;
  ...
  if (recent_tooltip) {
    if (window) window->hide();
    fltk::add_timeout(Fl_Tooltip::hoverdelay(), tooltip_timeout);
    ...
    if (window && window->visible()) window->hide();
    fltk::add_timeout(Fl_Tooltip::delay(), tooltip_timeout);
  }
...
void fltk::Widget::tooltip(const char *tt) {
