...
fltk::Widget *Fl_Tabs::which(int event_x, int event_y) {
  int H = tab_height();
  if (H < 0) {
...
int Fl_Tabs::handle(int event) {
  ...
  case FL_DRAG:
  case FL_RELEASE:
      o = which(fltk::event_x(), fltk::event_y());
    if (event == FL_RELEASE) {
      push(0);
      if (o && fltk::visible_focus() && fltk::focus()!=this) { 
        fltk::focus(this);
        redraw_tabs();
      }
      if (o && value(o)) {
  ...
  case FL_MOVE: {
    ...
    if ( (H>=0) && (fltk::event_y()>y()+H) )
      return ret;
    else if ( (H<0) && (fltk::event_y() < y()+h()+H) )
      return ret;
    else { 
      n = which(fltk::event_x(), fltk::event_y());
      if (!n) n = this;
    }
    if (n!=o)
      Fl_Tooltip::enter(n);
...
int Fl_Tabs::push(fltk::Widget *o) {
  if (push_ == o) return 0;
...
*/
int Fl_Tabs::value(fltk::Widget *newvalue) {
  fltk::Widget*const* a = array();
  int ret = 0;
  for (int i=children(); i--;) {
    fltk::Widget* o = *a++;
...
void Fl_Tabs::draw_tab(int x1, int x2, int W, int H, fltk::Widget* o, int what) {
  int sel = (what == SELECTED);
  int dh = fltk::box_dh(box());
  int dy = fltk::box_dy(box());
  char prev_draw_shortcut = fl_draw_shortcut;
  fl_draw_shortcut = 1;
