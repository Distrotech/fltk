...
#elif defined(WIN32)
    if (!gl_choice) fltk::gl_visual(0);
    context = fl_create_gl_context(fltk::Window::current(), gl_choice);
...
#  error Unsupported platform
  ...
  fl_set_gl_context(fltk::Window::current(), context);
#if !defined(WIN32) && !defined(__APPLE__)
  glXWaitX();
#endif
  if (pw != fltk::Window::current()->w() || ph != fltk::Window::current()->h()) {
    pw = fltk::Window::current()->w();
    ph = fltk::Window::current()->h();
    glLoadIdentity();
  ...
  if (clip_state_number != fl_clip_state_number) {
    ...
    int x, y, w, h;
    if (fl_clip_box(0, 0, fltk::Window::current()->w(), fltk::Window::current()->h(),
		    x, y, w, h)) {
      fl_clip_region(XRectangleRegion(x,y,w,h));
      glScissor(x, fltk::Window::current()->h()-(y+h), w, h);
...
int fltk::gl_visual(int mode, int *alist) {
