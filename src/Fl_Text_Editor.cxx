...
int Fl_Text_Editor::handle_key() {
  ...
  if (fltk::compose(del)) {
    ...
    kill_selection(this);
    if (fltk::event_length()) {
      if (insert_mode()) insert(fltk::event_text());
      else overstrike(fltk::event_text());
    }
  ...
  int key = fltk::event_key(), state = fltk::event_state(), c = fltk::event_text()[0];
