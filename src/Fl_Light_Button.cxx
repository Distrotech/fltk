...
// Subclass of fltk::Button where the "box" indicates whether it is
...
void Fl_Light_Button::draw() {
  if (box()) draw_box(this==fltk::pushed() ? fl_down(box()) : box(), color());
  ...
  if (down_box()) {
	...
	if (value()) {
      ...
      case _FL_ROUND_UP_BOX :
        ...
        draw_box(down_box(), x()+dx, y()+dy, W, W, FL_BACKGROUND2_COLOR);
	if (value()) {
	  int tW = (W - fltk::box_dw(down_box())) / 2 + 1;
    ...
    if (fltk::scheme() && !strcmp(fltk::scheme(), "plastic")) {
