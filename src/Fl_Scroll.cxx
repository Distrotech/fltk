...
void Fl_Scroll::fix_scrollbar_order() {
  fltk::Widget** a = (fltk::Widget**)array();
...
void Fl_Scroll::recalc_scrollbars(ScrollInfo &si) {
  ...
  si.innerbox_x = x()+fltk::box_dx(box());
  si.innerbox_y = y()+fltk::box_dy(box());
  si.innerbox_w = w()-fltk::box_dw(box());
  si.innerbox_h = h()-fltk::box_dh(box());
  ...
  // See if children would fit if we had no scrollbars...
  {
    ...
    si.scrollsize = scrollbar_size_ ? scrollbar_size_ : fltk::scrollbar_size();
    si.vneeded = 0;
    si.hneeded = 0;
    if (type() & VERTICAL) {
...
void Fl_Scroll::resize(int X, int Y, int W, int H) {
  int dx = X-x(), dy = Y-y();
  int dw = W-w(), dh = H-h();
  fltk::Widget::resize(X,Y,W,H); // resize _before_ moving children around
  fix_scrollbar_order();
...
void Fl_Scroll::hscrollbar_cb(fltk::Widget* o, void*) {
  Fl_Scroll* s = (Fl_Scroll*)(o->parent());
  s->scroll_to(int(((Fl_Scrollbar*)o)->value()), s->yposition());
}

void Fl_Scroll::scrollbar_cb(fltk::Widget* o, void*) {
  Fl_Scroll* s = (Fl_Scroll*)(o->parent());
...
hscrollbar(X,Y+H-fltk::scrollbar_size(),
           W-fltk::scrollbar_size(),fltk::scrollbar_size()) {
  type(BOTH);
  xposition_ = oldx = 0;
  yposition_ = oldy = 0;
  scrollbar_size_ = 0;
