...
fltk::SharedImage **fltk::SharedImage::images_ = 0;	// Shared images
int	fltk::SharedImage::num_images_ = 0;	// Number of shared images
int	fltk::SharedImage::alloc_images_ = 0;	// Allocated shared images

Fl_Shared_Handler *fltk::SharedImage::handlers_ = 0;// Additional format handlers
int	fltk::SharedImage::num_handlers_ = 0;	// Number of format handlers
int	fltk::SharedImage::alloc_handlers_ = 0;	// Allocated format handlers
...
/** Returns the fltk::SharedImage* array */
fltk::SharedImage **fltk::SharedImage::images() {
  return images_;
}
/** Returns the total number of shared images in the array. */
int fltk::SharedImage::num_images() {
  return num_images_;
}


//
// 'fltk::SharedImage::compare()' - Compare two shared images...
//

int
fltk::SharedImage::compare(fltk::SharedImage **i0,		// I - First image
                           fltk::SharedImage **i1) {	// I - Second image
...
*/
fltk::SharedImage::SharedImage() : Fl_Image(0,0,0) {
  name_        = 0;
  refcount_    = 1;
  original_    = 0;
  image_       = 0;
...
/** 
  ...
  from a program. Use the get() method instead.
*/
fltk::SharedImage::SharedImage(const char *n,		// I - Filename
...
//
// 'fltk::SharedImage::add()' - Add a shared image to the array.
//

void
fltk::SharedImage::add() {
  fltk::SharedImage	**temp;		// New image pointer array...

  if (num_images_ >= alloc_images_) {
    // Allocate more memory...
    temp = new fltk::SharedImage *[alloc_images_ + 32];

    if (alloc_images_) {
      memcpy(temp, images_, alloc_images_ * sizeof(fltk::SharedImage *));
  ...
  if (num_images_ > 1) {
    qsort(images_, num_images_, sizeof(fltk::SharedImage *),
...
//
// 'fltk::SharedImage::update()' - Update the dimensions of the shared images.
//

void
fltk::SharedImage::update() {
  if (image_) {
    w(image_->w());
    h(image_->h());
    d(image_->d());
...
/**
  ...
  used directly from a program. Use the fltk::SharedImage::release() method
  instead.
*/
fltk::SharedImage::~SharedImage() {
    ...
    if (images_[i] == this) {
      ...
      if (i < num_images_) {
        memmove(images_ + i, images_ + i + 1,
                (num_images_ - i) * sizeof(fltk::SharedImage *));
      }
...
//
// 'fltk::SharedImage::copy()' - Copy and resize a shared image...
//

Fl_Image *
fltk::SharedImage::copy(int W, int H) {
  Fl_Image		*temp_image;	// New image file
  fltk::SharedImage	*temp_shared;	// New shared image
  ...
  temp_shared = new fltk::SharedImage();
...
//
// 'fltk::SharedImage::color_average()' - Blend colors...
//

void
fltk::SharedImage::color_average(Fl_Color c,	// I - Color to blend with
...
//
// 'fltk::SharedImage::desaturate()' - Convert the image to grayscale...
//

void
fltk::SharedImage::desaturate() {
  if (!image_) return;
...
//
// 'fltk::SharedImage::draw()' - Draw a shared image...
//

void
fltk::SharedImage::draw(int X, int Y, int W, int H, int cx, int cy) {
...
//
// 'fltk::SharedImage::uncache()' - Uncache the shared image...
//

void fltk::SharedImage::uncache()
{
...
fltk::SharedImage* fltk::SharedImage::find(const char *n, int W, int H) {
  fltk::SharedImage	*key,		// Image key
			**match;	// Matching image

  if (num_images_) {
    key = new fltk::SharedImage();
    key->name_ = new char[strlen(n) + 1];
    strcpy((char *)key->name_, n);
    key->w(W);
    key->h(H);

    match = (fltk::SharedImage **)bsearch(&key, images_, num_images_,
                                          sizeof(fltk::SharedImage *),
                                        (compare_func_t)compare);
...
*/
fltk::SharedImage* fltk::SharedImage::get(const char *n, int W, int H) {
  fltk::SharedImage	*temp;		// Image

  if ((temp = find(n, W, H)) != NULL) return temp;

  if ((temp = find(n)) == NULL) {
    temp = new fltk::SharedImage(n);
  ...
  if ((temp->w() != W || temp->h() != H) && W && H) {
    temp = (fltk::SharedImage *)temp->copy(W, H);
    temp->add();
  }
...
void fltk::SharedImage::add_handler(Fl_Shared_Handler f) {
  int			i;		// Looping var...
...
void fltk::SharedImage::remove_handler(Fl_Shared_Handler f) {
  int	i;				// Looping var...
