...
*/
class ColorMenu : public fltk::Window {
  Fl_Color initial;
...
fltk::Window(BOXSIZE*8+1+2*BORDER, BOXSIZE*32+1+2*BORDER) {
  clear_border();
  set_modal();
...
int ColorMenu::handle(int e) {
  ...
  case FL_PUSH:
  case FL_DRAG: {
    int X = (fltk::event_x_root() - x() - BORDER);
    if (X >= 0) X = X/BOXSIZE;
    int Y = (fltk::event_y_root() - y() - BORDER);
    if (Y >= 0) Y = Y/BOXSIZE;
    if (X >= 0 && X < 8 && Y >= 0 && Y < 32)
      c = 8*Y + X;
    else
...
Fl_Color ColorMenu::run() {
  if (which < 0 || which > 255) {
    position(fltk::event_x_root()-w()/2, fltk::event_y_root()-y()/2);
  } else {
    position(fltk::event_x_root()-(initial%8)*BOXSIZE-BOXSIZE/2-BORDER,
             fltk::event_y_root()-(initial/8)*BOXSIZE-BOXSIZE/2-BORDER);
  }
  show();
  fltk::grab(*this);
  done = 0;
  while (!done) fltk::wait();
  fltk::grab(0);
  return which;
}
