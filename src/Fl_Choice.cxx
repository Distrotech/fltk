...
void Fl_Choice::draw() {
  int dx = fltk::box_dx(FL_DOWN_BOX);
  int dy = fltk::box_dy(FL_DOWN_BOX);
  int H = h() - 2 * dy;
  int W = (H > 20) ? 20 : H;
  int X = x() + w() - W - dx;
  int Y = y() + dy;
  ...
  if (mvalue()) {
    ...
    if ( fltk::scheme()) {
      ...
      fl_draw_shortcut = 0;
      if ( fltk::focus() == this ) draw_focus(box(), xx, yy, ww, hh);
    }
    else {
      fl_draw_shortcut = 2; // hack value to make '&' disappear
      m.draw(xx, yy, ww, hh, this, fltk::focus() == this);
      fl_draw_shortcut = 0;
    }
...
int Fl_Choice::handle(int e) {
  ...
  case FL_KEYBOARD:
      if (fltk::event_key() != ' ' ||
          (fltk::event_state() & (FL_SHIFT | FL_CTRL | FL_ALT | FL_META))) return 0;
  case FL_PUSH:
  ...
  J1:
    ...
    return 1;
  case FL_SHORTCUT:
      if (fltk::Widget::test_shortcut()) goto J1;
