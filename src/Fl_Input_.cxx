...
void Fl_Input_::drawtext(int X, int Y, int W, int H) {
  ...
  if (fltk::focus()!=this && !size()) {
    if (do_mu) { // we have to erase it if cursor was there
      draw_box(box(), X-fltk::box_dx(box()), Y-fltk::box_dy(box()),
               W+fltk::box_dw(box()), H+fltk::box_dh(box()), color());
    }
    return;
  }

  int selstart, selend;
  if (fltk::focus()!=this && /*fltk::selection_owner()!=this &&*/ fltk::pushed()!=this)
    selstart = selend = 0;
  ...
  for (p=value(), curx=cury=lines=0; ;) {
    ...
    if (position() >= p-value() && position() <= e-value()) {
      ...
      if (fltk::focus()==this && !was_up_down) up_down_pos = curx;
      cury = lines*height;
  ...
  CONTINUE2:
    // draw the cursor:
    if (fltk::focus() == this && selstart == selend &&
...
void Fl_Input_::handle_mouse(int X, int Y, int /*W*/, int /*H*/, int drag) {
  ...
  (fltk::event_y()-Y+yscroll_)/fl_height() : 0;
  ...
  const char *l, *r, *t; double f0 = fltk::event_x()-X+xscroll_;
  for (l = p, r = e; l<r; ) {
    double f;
    ...
    if (f <= fltk::event_x()) {l = t; f0 = fltk::event_x()-f;}
    else r = t-cw;
  }
  if (l < e) { // see if closer to character on right:
    ...
    int cw = fl_utf8len((char)l[0]);
    if (cw > 0) {
      f1 = X-xscroll_+expandpos(p, l + cw, buf, 0) - fltk::event_x();
      if (f1 < f0) l = l+cw;
    }
  }
  newpos = l-value();
...
int Fl_Input_::position(int p, int m) {
  ...
  //if (fltk::selection_owner() == this) fltk::selection_owner(0);
  if (p != m) {
    ...
    if (position_ == mark_) {
      ...
      if (fltk::focus() == this && !(damage()&FL_DAMAGE_EXPOSE)) {
...
/**
  ...
  \see fltk::copy(const char *, int, int)
*/
...
int Fl_Input_::copy_cuts() {
  ...
  fltk::copy(undobuffer, yankcut, 1);
  return 1;
}
...
int Fl_Input_::handletext(int event, int X, int Y, int W, int H) {
  ...
  case FL_FOCUS:
    ...
    } else //if (fltk::selection_owner() != this)
  ...
  case FL_PUSH:
    if (active_r() && window()) window()->cursor(FL_CURSOR_INSERT);
      ...
      handle_mouse(X, Y, W, H, fltk::event_state(FL_SHIFT));
  ...
  case FL_PASTE: {
    ...
    if (!fltk::event_text() || !fltk::event_length()) return 1;

    // strip trailing control characters and spaces before pasting:
    const char* t = fltk::event_text();
    const char* e = t+fltk::event_length();
...
/**
  ...
  \see fltk::Widget::resize(int, int, int, int)
*/
