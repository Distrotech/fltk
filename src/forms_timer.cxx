...
void Fl_Timer::step() {
  ...
  if (lastdelay > 0.0 && delay <= 0.0) {
    if (type() == FL_HIDDEN_TIMER) {
      ...
      redraw();
      fltk::add_timeout(FL_TIMER_BLINKRATE, stepcb, this);
    }
    set_changed();
    do_callback();
  } else {
...
*/
Fl_Timer::~Fl_Timer() {
  fltk::remove_timeout(stepcb, this);
}
...
void Fl_Timer::value(double d) {
  ...
  fltk::remove_timeout(stepcb, this);
  if (on) fltk::add_timeout(FL_TIMER_BLINKRATE, stepcb, this);
}
