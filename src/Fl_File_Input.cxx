...
void Fl_File_Input::draw() {
  ...
  fltk::focus()!=this && !size() && !(damage()&FL_DAMAGE_ALL);
  if ((damage() & FL_DAMAGE_ALL) || must_trick_fl_input_) 
    draw_box(b,x(),y()+DIR_HEIGHT,w(),h()-DIR_HEIGHT,color());
  if (!must_trick_fl_input_) 
    Fl_Input_::drawtext(x()+fltk::box_dx(b)+3, y()+fltk::box_dy(b)+DIR_HEIGHT,
                        w()-fltk::box_dw(b)-6, h()-fltk::box_dh(b)-DIR_HEIGHT);
}
...
//  printf("handle(event = %d)\n", event);
  ...
  switch (event) {
    ...
    case FL_PUSH :
      inButtonBar = (fltk::event_y() < (y() + DIR_HEIGHT));
    case FL_RELEASE :
    case FL_DRAG :
      if (inButtonBar) 
...
Fl_File_Input::handle_button(int event)		// I - Event
{
  ...
  for (X = 0, i = 0; buttons_[i]; i ++)
  {
    ...
    if (X > xscroll() && fltk::event_x() < (x() + X - xscroll())) break;
  }
