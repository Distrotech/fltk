//
// "$Id: input.d 4626 2005-11-02 13:57:56Z matt $"
//
...
private import fl.float_input;
private import fl.int_input;
private import fl.secret_input;
private import fl.multiline_input;
private import fl.button;
private import fl.toggle_button;
private import fl.color_chooser;

private import std.c.stdio;


void cb(Fl_Widget  ob) {
  printf("Callback for %s '%s'\n",ob.label(),(cast(Fl_Input)ob).value());
}

int when = 0;
Fl_Input  input[5];

void toggle_cb(Fl_Widget  o, int v) {
  if ((cast(Fl_Toggle_Button )o).value()) when |= v; else when &= ~v;
...
void color_cb(Fl_Widget button, void* v) {
  ...
  ubyte r,g,b; Fl.get_color(c, r,g,b);
  if (fl_color_chooser_ub(null,r,g,b)) {
...
int main(char[][] args) {
  ...
  b.color(input[0].color()); b.callback(&color_cb, cast(void*)0);
  b.tooltip("Color behind the text");
  b = new Fl_Button(220,y1,100,25,"selection_color"); y1 += 25;
  b.color(input[0].selection_color()); b.callback(&color_cb, cast(void*)1);
  ...
  b.color(input[0].textcolor()); b.callback(&color_cb, cast(void*)2);
  b.labelcolor(fl_contrast(FL_BLACK,b.color()));
...
//
// End of "$Id: input.d 4626 2005-11-02 13:57:56Z matt $".
//
