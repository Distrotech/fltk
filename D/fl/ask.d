  ...
  char *b2)
{
  ...
  char buffer[1024];
  if (!strcmp(fmt,"%s")) {
    message.label((cast(char**)ap)[1]);
  } else {
    vsnprintf(buffer, 1024, fmt, cast(char*)ap);
  ...
  for (;;) {
    ...
    if (!o) Fl.wait();
    else if (o is button[0]) {r = 0; break;}
    else if (o is button[1]) {r = 1; break;}
    else if (o is button[2]) {r = 2; break;}
    else if (o is message_form) {r = 0; break;}
  }
  message_form.hide();
...
static char* input_innards(char* fmt, void* ap,
