//
// "$Id: fl_boxtype.cxx 5190 2006-06-09 16:16:34Z mike $"
//
...
ubyte* fl_gray_ramp() {
  if (draw_it_active)
    return active_ramp.ptr-'A';
  else
    return inactive_ramp.ptr-'A';
}
...
struct Fl_Boxtable_Entry {
  Fl_Box_Draw_F f;
  ubyte dx, dy, dw, dh;
  int set;
};

Fl_Boxtable_Entry[256] fl_box_table= [
// must match list in Enumerations.H!!!
  {&fl_no_box,		0,0,0,0,1},		
  {&fl_rectf,		0,0,0,0,1}, // FL_FLAT_BOX
  {&fl_up_box,		2,2,4,4,1},
  {&fl_down_box,	2,2,4,4,1},
  {&fl_up_frame,	2,2,4,4,1},
  {&fl_down_frame,	2,2,4,4,1},
