...
static char ctrl(char c) { return (c)^0x40; }
...
private:
  int handle_key() {
      ...
      if (input_type() == FL_FLOAT_INPUT || input_type() == FL_INT_INPUT) {
        ...
        version (HAVE_LOCALECONV) {
          if (!legal_fp_chars) {
            ...
            char *chars = cast(char*)malloc(len+1);
    ...
    switch (ascii) {
    case 'A'^0x40:
      return shift_caret(line_start(caret())) + NORMAL_INPUT_MOVE;
    case 'B'^0x40:
      return shift_caret(caret()-1) + NORMAL_INPUT_MOVE;
    case 'C'^0x40: // copy
      return copy(1);
    case 'D'^0x40:
    case '?'^0x40:
    ...
    case 'E'^0x40:
      return shift_caret(line_end(caret())) + NORMAL_INPUT_MOVE;
    case 'F'^0x40:
      return shift_caret(caret()+1) + NORMAL_INPUT_MOVE;
    case 'H'^0x40:
      if (readonly()) {
    ...
    case 'P'^0x40:
      ...
      shift_up_down_caret(line_start(i));
      return 1;
    case 'U'^0x40:
      if (readonly()) {
        fl_beep();
  ...
  int shift_caret(int p) {
    return caret(p, Fl.event_state(FL_SHIFT) ? mark() : p);
  }

  int shift_up_down_caret(int p) {
    return up_down_caret(p, Fl.event_state(FL_SHIFT));
  }
...
public:
  ...
  int handle(int event) {
    static int dnd_save_caret, dnd_save_mark, drag_start = -1, newpos;
    ...
    case FL_FOCUS:
      ...
      case 0xfe20: // XK_ISO_Left_Tab
        ...
        break;
      default:
        caret(caret(),mark());// turns off the saved up/down arrow caret
        break;
      }
      break;
    ...
    case FL_KEYBOARD:
      if (Fl.event_key() == FL_Tab && mark() != caret()) {
        // Set the current cursor caret to the end of the selection...
        if (mark() > caret())
          caret(mark());
        else
          caret(caret());
    ...
    case FL_PUSH:
      if (Fl.dnd_text_ops()) {
        ...
        if (Fl.focus() is this && !Fl.event_state(FL_SHIFT) && input_type()!=FL_SECRET_INPUT &&
    ...
    case FL_DRAG:
  	...
  	// save the caret because sometimes we don't get DND_ENTER:
  	dnd_save_caret = caret();
  	dnd_save_mark = mark();
  	// drag the data:
  	copy(0); Fl.dnd();
  	return 1;
    ...
    case FL_RELEASE:
      if (Fl.event_button() == 2) {
        ...
        // user clicked in the field and wants to reset the cursor caret...
    ...
    case FL_DND_ENTER:
      ...
      if (!(dnd_save_focus is this)) {
        Fl.focus(this);
        handle(FL_FOCUS);
      }
      // fall through:
    case FL_DND_DRAG: 
      //int p = mouse_caret(X, Y, W, H);
      {
        Fl_Boxtype b = box();
        super.handle_mouse(
          x()+Fl.box_dx(b), y()+Fl.box_dy(b),
    ...
    case FL_DND_LEAVE:
      caret(dnd_save_caret, dnd_save_mark);
      if (!(dnd_save_focus is this)) {
