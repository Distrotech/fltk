...
int fl_measure_pixmap(/*const*/ char* const* data, int &w, int &h) {
  return fl_measure_pixmap((char*const*)data,w,h);
}
...
static void cb1(void*v, int x, int y, int w, ubyte* buf) {
...
static void cb2(void*v, int x, int y, int w, ubyte* buf) {
  pixmap_data& d = *(pixmap_data*)v;
  ubyte* p = d.data[y]+2*x;
  U64* q = (U64*)buf;
  for (int X=w; X>0; X-=2) {
...
ubyte **fl_mask_bitmap; // if non-zero, create bitmap and store pointer here

int fl_draw_pixmap(/*const*/ char* const* data, int x,int y,Fl_Color bg) {
  return fl_draw_pixmap((char*const*)data,x,y,bg);
}

int fl_draw_pixmap(char*const* di, int x, int y, Fl_Color bg) {
  pixmap_data d;
  if (!fl_measure_pixmap(di, d.w, d.h)) return 0;
  ubyte*const* data = (ubyte*const*)(di+1);
...
version (U64) {
	...
	c = (ubyte *)&d.colors[ind];
      }
      // look for "c word", or last word if none:
      ubyte *previous_word = p;
      for (;;) {
	while (*p && isspace(*p)) p++;
	ubyte what = *p++;
	while (*p && !isspace(*p)) p++;
...
version (__APPLE_QUARTZ__) {
      ...
      if (!fl_parse_color((char*)p, c[0], c[1], c[2])) {
...
version (!__APPLE_QUARTZ__) {
  ...
  if (fl_mask_bitmap && transparent_index >= 0) {
    ...
    *fl_mask_bitmap = bitmap;
	...
	int dw = d.w;
	for (int X = 0; X < W; X++) {
	  ubyte b = (dw-->0 && *p++ != transparent_index);
