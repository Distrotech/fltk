...
const int III = 255;  // maximum intensity of the basic colors
...
const int FL_GRAY_RAMP = 32; 
const int FL_NUM_GRAY = 24; 
const int FL_GRAY = 49;  // old value is 47
alias ubyte ubyte;
#include <math.h>
...
void background(ubyte r, ubyte g, ubyte b) {
  ...
  for (int i = 0; i < FL_NUM_GRAY; i++) {
    ...
    cmap[i+FL_GRAY_RAMP][0] = ubyte(pow(gray,powr)*255+.5);
    cmap[i+FL_GRAY_RAMP][1] = ubyte(pow(gray,powg)*255+.5);
    cmap[i+FL_GRAY_RAMP][2] = ubyte(pow(gray,powb)*255+.5);
  }
}
