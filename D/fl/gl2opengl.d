...
alias float Matrix[4][4];
void pushmatrix() {glPushMatrix();}
void popmatrix() {glPopMatrix();}
void multmatrix(Matrix m) {glMultMatrixf((float *)m);}
void color(int n) {glIndexi(n);}
void rect(int x,int y,int r,int t) {gl_rect(x,y,r-x,t-y);}
void rectf(int x,int y,int r,int t) {glRectf(x,y,r+1,t+1);}
void recti(int x,int y,int r,int t) {gl_rect(x,y,r-x,t-y);}
void rectfi(int x,int y,int r,int t) {glRecti(x,y,r+1,t+1);}
void rects(int x,int y,int r,int t) {gl_rect(x,y,r-x,t-y);}
void rectfs(int x,int y,int r,int t) {glRects(x,y,r+1,t+1);}
