...
extern "C" {
version (!HAVE_SCANDIR) {
  int fl_scandir (char *dir, dirent ***namelist,
	          int (*select)(dirent *),
	          int (*compar)(dirent **, dirent **));
const int scandir = fl_scandir; 
}
}
...
int fl_filename_list(char *d, dirent ***list,
                     Fl_File_Sort_F  sort) {
version (!HAVE_SCANDIR) {
  int n = scandir(d, list, 0, sort);
} else version (__hpux) || defined(__CYGWIN__) {
  // HP-UX, Cygwin define the comparison function like this:
  int n = scandir(d, list, 0, (int(*)(dirent **, dirent **))sort);
} else version (__osf__) {
  // OSF, DU 4.0x
  int n = scandir(d, list, 0, (int(*)(dirent **, dirent **))sort);
} else version (_AIX) {
