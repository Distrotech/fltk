...
/+=
  ...
  int replace(int, int, char*, int=0);
  int cut() {return replace(position(), mark(), 0);}
  int cut(int n) {return replace(position(), position()+n, 0);}
  int cut(int a, int b) {return replace(a, b, 0);}
  int insert(char* t, int l=0){return replace(position_, mark_, t, l);}
  ...
  Fl_Color cursor_color() {return (Fl_Color)cursor_color_;}
  void cursor_color(uint n) {cursor_color_ = n;}

  int input_type() {return type() & FL_INPUT_TYPE; }
  void input_type(int t) { type((ubyte)(t | readonly())); }
  int readonly() { return type() & FL_INPUT_READONLY; }
  void readonly(int b) { if (b) type((ubyte)(type() | FL_INPUT_READONLY));
                         else type((ubyte)(type() & ~FL_INPUT_READONLY)); }
...
extern void fl_draw(char*, int, float, float);
...
const char* Fl_Input_.expand(char* p, char* buf) {
  char* o = buf;
  char* e = buf+(MAXBUF-4);
  char* lastspace = p;
...
int Fl_Input_.word_start(int i) {
...
int Fl_Input_.line_start(int i) {
...
int Fl_Input_.replace(int b, int e, char* text, int ilen) {
...
int Fl_Input_.static_value(char* str, int len) {
  clear_changed();
...
int Fl_Input_.static_value(char* str) {
  return static_value(str, str ? strlen(str) : 0);
}

int Fl_Input_.value(char* str, int len) {
  int r = static_value(str, len);
  if (len) put_in_buffer(len);
  return r;
}

int Fl_Input_.value(char* str) {
