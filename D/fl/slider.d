...
const Fl_Type FL_VERT_SLIDER		= 0;
const Fl_Type FL_HOR_SLIDER		= 1;
const Fl_Type FL_VERT_FILL_SLIDER	= 2;
const Fl_Type FL_HOR_FILL_SLIDER	= 3;
const Fl_Type FL_VERT_NICE_SLIDER	= 4;
const Fl_Type FL_HOR_NICE_SLIDER	= 5;
...
protected:
  ...
  void draw(int X, int Y, int W, int H) {
    ...
    if (!box1) {box1 = cast(Fl_Boxtype)(box()&-2); if (!box1) box1 = FL_UP_BOX;}
    if (type() == FL_VERT_NICE_SLIDER) {
  ...
  int handle(Fl_Event event, int X, int Y, int W, int H) {
    switch (event) {
    case FL_PUSH:
...
public:
  ...
  this(Fl_Type t, int X, int Y, int W, int H, char[] l=null) {
    super(X, Y, W, H, l);
    type(t);
  ...
  void slider_size(double v) {
    if (v <  0) v = 0;
    if (v > 1) v = 1;
    if (slider_size_ != cast(float)(v)) {
      slider_size_ = cast(float)(v); 
      damage(FL_DAMAGE_EXPOSE);
    }
  }
...
//
// End of "$Id: slider.d 5190 2006-06-09 16:16:34Z mike $".
//
