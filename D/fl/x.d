...
version (WIN32) {
  static assert (0, "No Win32 support yet");
  public import fl.win32;
} else version (__APPLE__) {
  public import fl.mac;
} else {
  static assert (0, "No X11 support yet");
/+
version (_ABIN32) || defined(_ABI64) // fix for broken SGI Irix X .h files {
#      pragma set woff 3322
}
#    include <X11/Xlib.h>
#    include <X11/Xutil.h>
version (_ABIN32) || defined(_ABI64) {
...
// is only here so that fl_xid can be declared :
class Fl_X {
public:
  Window xid;
  Window other_xid;
  ...
  static Fl_X  first;
  static Fl_X  i(Fl_Window  wi) {return wi.i;}
...
Window fl_xid(Fl_Window w) {return Fl_X.i(w)->xid;}
...
//
// "$Id: x.d 5201 2006-06-14 11:01:03Z matt $"
//
...
const int CONSOLIDATE_MOTION = 1; 
...
void Fl.get_mouse(int &xx, int &yy) {
  ...
  Window c; int mx,my,cx,cy; uint mask;
                                 ...
                                 uint d4=0)
{
  ...
  e.xclient.format = 32;
  e.xclient.data.l[0] = (int)d0;
  e.xclient.data.l[1] = (int)d1;
  e.xclient.data.l[2] = (int)d2;
  e.xclient.data.l[3] = (int)d3;
  e.xclient.data.l[4] = (int)d4;
...
int fl_handle(XEvent& thisevent)
{
  ...
  case SelectionNotify: {
	...
	buffer = (ubyte*)realloc(buffer, bytesread+count*format/8+remaining);
  ...
  case SelectionRequest: {
    ...
    if (e.target == TARGETS) {
      ...
      XChangeProperty(fl_display, e.requestor, e.property,
		      XA_ATOM, sizeof(Atom)*8, 0, (ubyte*)&a,
		      sizeof(Atom));
    } else if (/*e.target == XA_STRING &&*/ fl_selection_length[clipboard]) {
      XChangeProperty(fl_display, e.requestor, e.property,
		      e.target, 8, 0,
		      (ubyte *)fl_selection_buffer[clipboard],
...
//    XFree(x);
  ...
  case ClientMessage: {
    ...
    int* data = fl_xevent.xclient.data.l;
...
#  if BACKSPACE_HACK
    ...
    if (keysym >= 0xff91 && keysym <= 0xff9f) {
      ...
      // numlock state...
      uint keysym1 = XKeycodeToKeysym(fl_display, keycode, 1);
...
static int childEventMask = ExposureMask;

static int XEventMask =
...
version (FL_CENTER_WINDOWS) {
  ...
  if (!win.parent() && !attr.override_redirect) {
    ...
    XChangeProperty(fl_display, xp.xid, WM_PROTOCOLS,
 		    XA_ATOM, 32, 0, (ubyte*)&WM_DELETE_WINDOW, 1);
    ...
    if (!win.border()) {
      ...
      XChangeProperty (fl_display, xp.xid, net_wm_state, XA_ATOM, 32, 
          PropModeAppend, (ubyte*) &net_wm_state_skip_taskbar, 1);
    }

    // Make it receptive to DnD:
    int version = 4;
    XChangeProperty(fl_display, xp.xid, fl_XdndAware,
		    XA_ATOM, sizeof(int)*8, 0, (ubyte*)&version, 1);
...
void Fl_Window.label(char *name,char *iname) {
  ...
  if (shown() && !parent()) {
    ...
    XChangeProperty(fl_display, i.xid, XA_WM_NAME,
		    XA_STRING, 8, 0, (ubyte*)name, strlen(name));
    if (!iname) iname = fl_filename_name(name);
    XChangeProperty(fl_display, i.xid, XA_WM_ICON_NAME, 
		    XA_STRING, 8, 0, (ubyte*)iname, strlen(iname));
  }
}
...
//
// End of "$Id: x.d 5201 2006-06-14 11:01:03Z matt $".
//
